...
/*   Updated: 2023/01/19 01:29:49 by astalha          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    copy(t_infos *my_infos)
{
    ...
    i = 0;
    my_infos->copy = (char **) malloc((matlen(my_infos->matrix) + 1) * sizeof(char *));
    if (!my_infos->copy)
        return ;
...
int     valid_path(t_infos  *my_infos)
{
    ...
    get_pos(my_infos, 'E');
    if (my_infos->copy[my_infos->xp - 1][my_infos->yp] != 'F' && my_infos->copy[my_infos->xp + 1][my_infos->yp] != 'F'
        && my_infos->copy[my_infos->xp][my_infos->yp - 1] != 'F' && my_infos->copy[my_infos->xp][my_infos->yp + 1] != 'F')
        return (ft_putstr_fd("Error : no valid path, player can't exit\n", 2), 0);
    return 1;
}